...
void Entity::AIWander()
{
    ...
    {
        mDirection = getAlternateDirection();

        switch (mDirection)
        {
        case LEFT:  moveGridLeft();  break;
        case RIGHT: moveGridRight(); break;
        case UP:    moveGridUp();    break;
        case DOWN:  moveGridDown();  break;
        }
        return;
    }
...
void Entity::AIFollow(Entity* target)
{
    ...
    case WALKING:
        ...
        {
            if (mPosition.x > target->getPosition().x) moveGridLeft();
            else                                       moveGridRight();
        }
        else
        {
            if (mPosition.y > target->getPosition().y) moveGridUp();
            else                                       moveGridDown();
        }
        break;
...
// switching from the random bounce since we are using grid system
Direction Entity::getAlternateDirection() const
{
    ...
    int index = GetRandomValue(0, static_cast<int>(options.size()) - 1);
    return options[index];
}

// grid based versions of moving
void Entity::moveGridUp()
{
    mMovement = { 0.0f, -1.0f };
    mDirection = UP;
}

void Entity::moveGridDown()
{
    mMovement = { 0.0f, 1.0f };
    mDirection = DOWN;
}

void Entity::moveGridLeft()
{
    mMovement = { -1.0f, 0.0f };
    mDirection = LEFT;
}

void Entity::moveGridRight()
{
    mMovement = { 1.0f, 0.0f };
    mDirection = RIGHT;
}

void Entity::stopGridMovement()
{
    mMovement = { 0.0f, 0.0f };
}
